...
export async function getPurchaseLocalBySupplier(supplier_id: string): Promise<{ data: GetPurchaseLocalData[]; pagination: any }> {
  const baseUrl = process.env.NEXT_PUBLIC_API_URL;
  const token = localStorage.getItem('token');

  const res = await fetch(`${baseUrl}purchase/purchase-orders.php?type=local&supplier_id=${supplier_id}&status=approved&page=1&limit=1000`, {
    method: 'GET',
  ...
  if (json.status_code !== 200) {
    throw new Error(json.status_message || 'Failed to fetch local purchase orders by supplier');
  }
