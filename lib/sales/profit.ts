...
export async function exportsalesProfitExcel(profit_id: string): Promise<Blob> {
    const baseUrl = process.env.NEXT_PUBLIC_API_URL;
    const token = localStorage.getItem("token");

    const res = await fetch(`${baseUrl}sales/report/profit-summary.php?profit_id=${profit_id}`, {
